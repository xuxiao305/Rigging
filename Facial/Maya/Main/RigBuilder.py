...
class Facial:
    ...
    def loadAllCorrectivePose(self, root):
        print '\nLoading All Corrective Pose...'
        elemAllCorrectivePose = root.getElementsByTagName("CorrectivePoses")[0]
        for elemCorrectivePose in elemAllCorrectivePose.childNodes:
            if elemCorrectivePose.nodeType != 1:
                continue
            correctivePose = CorrectivePose(self.poseManager, elemCorrectivePose.tagName)

            if elemCorrectivePose.hasAttribute('remapFrom'):
                remapFromRange = eval(elemCorrectivePose.getAttribute('remapFrom'))
                correctivePose.setRemapFrom(remapFromRange)

            if elemCorrectivePose.hasAttribute('remapTo'):
                remapToRange = eval(elemCorrectivePose.getAttribute('remapTo'))
                correctivePose.setRemapFrom(remapToRange)


            # Load the linked trigger poses
            for elemTriggerPose in elemCorrectivePose.childNodes:
                if elemTriggerPose.nodeType != 1:
                    continue

                triggerWeight = 1.0
                if elemTriggerPose.hasAttribute('triggerWeight'):
                    triggerWeight = float(elemTriggerPose.getAttribute('triggerWeight'))

                triggerPose = self.poseManager.getPoseByName(elemTriggerPose.tagName)

                correctivePose.addNewTriggerPose(triggerPose, triggerWeight)

            self.poseManager.allCorrectivePoseList.append(correctivePose)
    ...
    def createBlendshapeHub(self):
        ...
        self.blendShapeHub = pm.ls(blendShapeNodeName)[0]

        num = 0
        for pose in self.poseManager.allPoseList:
            print str(pose.shapeMesh)
            cmd.blendShape(blendShapeNodeName, edit=True, target=(str(self.facialMesh), num, str(pose.shapeMesh), 1))
            pose.weightSolver.outWeight.connect(self.blendShapeHub.weight[num])
            num += 1

        for correctivePose in self.poseManager.allCorrectivePoseList:
            print 'correctivePose --->', correctivePose.name
            triggerPoseList = correctivePose.getTriggerPoseList()

            print 'trigger pose list ---->', triggerPoseList

            for triggerPose in triggerPoseList:
                print 'triggerPose ', triggerPose.name
                correctivePose.connectTriggerPoseWeight(triggerPose)

            cmd.blendShape(blendShapeNodeName, edit=True,
                       target=(str(self.facialMesh), num, str(correctivePose.shapeMesh), 1))

            correctivePose.triggerNode.outWeight.connect(self.blendShapeHub.weight[num])
            num += 1
    ...
    def reconstructSampler(self, rigMode):
        ...
        self.loadAllCorrectivePose(root)
...
class SampleGroup:
    ...
    def setupSampleDataFlow(self, valueBlender):
        ...
        for i in range(numPose):
            ...
            pose = self.facial.poseManager.allPoseList[poseIndexInAll]
            ...
            if poseIndex != -1:
                ...
                if valueBlendMode == 0:
                    weightAttr = pose.weightSolver.inWeightList
                else:
                    weightAttr = pose.weightSolver.inWeightMulList
...
class PoseManager:
    allPoseList = []
    allCorrectivePoseList = []
    editNodeList = []
    jointManager = 0

    def __init__(self, jointManager):
        self.allPoseList = []
        self.allCorrectivePoseList = []
        self.jointManager = jointManager
    ...
    def getCorrectivePoseIndexByName(self, name):
        for i in range(len(self.allCorrectivePoseList)):
            pose = self.allCorrectivePoseList[i]
            if pose.name == name:
                return i
        return -1
...
# Data class for corrective pose
class CorrectivePose:
    name = 0
    jointList = []
    active = False
    ...
    def setRemapFrom(self, remapFromRange):
        self.triggerNode.setAttr('remapFromX', remapFromRange[0])
        self.triggerNode.setAttr('remapFromY', remapFromRange[1])

    def setRemapTo(self, remapToRange):
        self.triggerNode.setAttr('remapToX', remapToRange[0])
        self.triggerNode.setAttr('remapToY', remapToRange[1])

    def addNewTriggerPose(self, triggerPose, triggerWeight):
        num = pm.Attribute.numElements(self.triggerNode.triggerPoseInfoList)
        self.triggerNode.setAttr('triggerPoseInfoList[%s].triggerPoseName' % str(num), triggerPose.name)
        self.triggerNode.setAttr('triggerPoseInfoList[%s].triggerWeight' % str(num), triggerWeight)

    def connectTriggerPoseWeight(self, triggerPose):
        index = self.getTriggerPoseIndexByName(triggerPose.name)

        triggerPose.weightSolver.outWeight.connect(self.triggerNode.triggerPoseInfoList[index].currentWeight)

    def getTriggerPoseIndexByName(self, name):
        num = pm.Attribute.numElements(self.triggerNode.triggerPoseInfoList)
        for p in range(num):
            triggerPoseName = self.triggerNode.getAttr('triggerPoseInfoList[%s].triggerPoseName' % str(p))
            if triggerPoseName == name:
                return p

    def getTriggerPoseList(self):
        lst = []

        allNum = pm.Attribute.numElements(self.triggerNode.triggerPoseInfoList)

        for p in range(allNum):
            triggerPoseName = self.triggerNode.getAttr('triggerPoseInfoList[%s].triggerPoseName' % str(p))
            triggerPose = self.poseManager.getPoseByName(triggerPoseName)
            lst.append(triggerPose)
